...
private:
	...
	void circle_pressed() { show(circle); next(); }
	void square_pressed() { show(square); next(); }
	void equal_tri_pressed() { show(equal_tri); next(); }
	void hexagon_pressed() { show(hexagon); next(); }
	void next();
	void quit();
...
Shape_window::Shape_window(Point xy, int w, int h, const string& title)
	...
	next_x{ Point{ x_max() - 230, 0 }, 50, 20, "next x:" },
	next_y{ Point{ x_max() - 130, 0 }, 50, 20, "next y:" },
